...
import { useLazyGetSmartDataSSOQuery } from 'services';
...
export const AnaliticsPage = () => {
  ...
  const [fetchSSO, { isSuccess }] = useLazyGetSmartDataSSOQuery();
  ...
  useEffect(() => {
    if (isSuccess && hashMd5) {
      localStorage.setItem('hashMd5', hashMd5);
      window.location.href = iframeParamUrl;
    }
  }, [hashMd5, iframeParamUrl, isSuccess]);

  return (
    <Main>
      <MainTitle>Аналитика</MainTitle>
      <IframeWrapper>
        <Iframe src={`${smartanalyticsIframe}/ru/#!/index/2065/reports/2/8184`}></Iframe>
      </IframeWrapper>
    </Main>
  );
};
